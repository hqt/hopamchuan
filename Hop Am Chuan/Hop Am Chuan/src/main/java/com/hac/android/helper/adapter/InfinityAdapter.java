...
public class InfinityAdapter extends BaseAdapter {
    ...
    public static String TAG = LogUtils.makeLogTag(BaseAdapter.class);
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        LogUtils.LOGE(TAG, "Get View at " + position);
        if ((position + 1 == mAdapter.getCount()) || (position == mAdapter.getCount())) {
            LogUtils.LOGE(TAG, "Unsuccessfully " + position + " has came to End Screen");
            if (isRunningBackground && !isLoading()) {
                ...
                Thread t = new Thread(new Runnable() {
                    @Override
                    public void run() {
                        LogUtils.LOGE(TAG, "Update Song on " + NetworkUtils.getThreadSignature());
    ...
    private class LoadingHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            LogUtils.LOGE(TAG, "Update Adapter on " + NetworkUtils.getThreadSignature());
