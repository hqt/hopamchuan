...
public class MusicPlayerController {
    ...
    private static String TAG = makeLogTag(MusicPlayerController.class);
    ...
    private void setupPauseResume() {
        if (player == null) return;
        if (player.isPlaying()) {
            LOGE(TAG, "Pause Media Player");
            player.pause();
        } else {
            LOGE(TAG, "Start Media Player");
            player.start();
        }
        updatePausePlay();
    }
    ...
    private View.OnClickListener mPauseListener = new View.OnClickListener() {
        public void onClick(View v) {
            LOGE(TAG, "Pause/Play Button is pressed");
            setupPauseResume();
            show(sDefaultTimeout);
        }
    };
